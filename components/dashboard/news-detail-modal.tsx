...
export function NewsDetailModal({
  ...
  const related = article
    ? allArticles.filter((n) => n.department === article.department && n.id !== article.id).slice(0, 3)
    : []
          ...
          <div className="max-h-[60vh] overflow-y-auto p-5">
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              ...
              <span className="inline-flex items-center gap-1">
                ...
                {article.department}
                {article.municipality ? `, ${article.municipality}` : ''}
              </span>
              <span className="inline-flex items-center gap-1">
                  ...
                  {related.map((r) => (
                    <div key={r.id} className="rounded-lg border border-border bg-background/40 p-2.5">
                      ...
                      <p className="mt-0.5 text-[11px] text-muted-foreground">
                        {r.source} · <span suppressHydrationWarning>{formatTimeAgo(r.publishedAt)}</span>
                      </p>
                    </div>
                  ))}
                </div>
